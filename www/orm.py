...
因为数据库中每张表的字段都不一样，所以我们需要动态的生成类,此时元类派上用场
类是对象的模板，元类是类的模板。我们的User类继承自Model类，而Model类的模板是元类ModelMetaClass,
所以当使用者实例化一个User类（一张表）的时候，User会根据Model去创建，而Model则根据ModelMetaClass动态创建，
...
# 任何定义了__metaclass__属性或指定了metaclass的都会通过元类定义的构造方法构造类
class ModelMetaclass(type):
    def __new__(cls, name, bases, attrs):
        ...
        # 从类属性中删除已加入映射字典的键,避免后面实例属性重名覆盖掉，个人觉得不删除也无所谓。。。
...
# ModelMetaclass元类只是定制了相关属性，而Model类就提供了操作数据库要用到的方法
